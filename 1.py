...
hero1 = 'qwertyuiopasdfghjklzxcvbnm1234567890'
...
def Lev ():
	...
	while True:
	   user = str("".join(random.choice(hero1)for x in range(1)))
	   user1 = str("".join(random.choice(hero1)for x in range(1)))
	   user2 = str("".join(random.choice(hero1)for x in range(1)))
	   user3 = str("".join(random.choice(hero1)for x in range(1)))
	   user5 = (user1+user2+'.'+user3)
	   user6 = (user1+'_'+user2+user3)
	   user7 = (user+user1+user2+user3)
	   user8 = (user3+user1+user2+user)
	   ...
	   if '"statusCode":10221,' in reqsnd:
	           ...
	           requests.post(f"https://api.telegram.org/bot{token}/sendMessage?chat_id={id}&text=Yo Nigga is Done : @{user}\nAfter | 300280")
	   else:
